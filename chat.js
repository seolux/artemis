...
const CHAT_RESPONSES = {
  ...
  seo: [/seo|référencement|optimis/i, () => {
    ...
    return `📊 **Score SEO global : ${s.globalScore}/100**\n\n${s.categories.map(c =>
      `• ${c.name} : **${c.score}/100** ${c.score >= 70 ? '✅' : c.score >= 40 ? '⚠️' : '❌'}`
  ...
  social: [/social|facebook|instagram|linkedin|tiktok|réseau/i, () => {
    ...
    ).join('\n')}\n\n⚠️ LinkedIn et TikTok sont absents. Le dernier post Facebook date du ${socs[0].lastPost}.`;
  }],
  // Certification
  ...
  resume: [/résumé|résume|overview|synthèse|récap|tout/i, () =>
    `📋 **Synthèse ${COMPANY_DATA.name} :**\n\n• 🏢 ${COMPANY_DATA.legalName} — ${COMPANY_DATA.formeJuridique}\n• 📍 ${COMPANY_DATA.locations.length} emplacements à Luxembourg\n• 👥 ${COMPANY_DATA.employees} employés\n• ⭐ ${COMPANY_DATA.googleBusiness.rating}/5 (${COMPANY_DATA.googleBusiness.totalReviews} avis Google)\n• 🔍 SEO : ${COMPANY_DATA.seo.globalScore}/100\n• 📍 GEO : ${COMPANY_DATA.geo.globalScore}/100\n• 🛡️ Certification : ${COMPANY_DATA.certification.level.toUpperCase()} (${COMPANY_DATA.certification.score}/100)\n• 📊 ${COMPANY_DATA.sources.reduce((a,s)=>a+s.dataPoints,0)} data points collectés depuis ${COMPANY_DATA.sources.length} sources`
  ],
  // Sources
  sources: [/source|données|data|scrap|collecte/i, () => {
    ...
    return `📡 **${srcs.length} sources analysées — ${total} data points :**\n\n${srcs.map(s =>
      `• **${s.name}** — ${s.dataPoints} points ${s.status === 'ok' ? '✅' : '⚠️'}`
    ).join('\n')}`;
  }]
};
